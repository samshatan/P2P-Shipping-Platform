...
export function PriceComparison({ 
  ...
  const { data, isLoading, error } = useQuery<RatesResponse>({
    ...
    queryFn: async () => {
      const response = await axios.get(`http://localhost:3001/couriers/rates`, {
        params: {
          pickup_pincode,
          delivery_pincode,
          weight_grams,
    ...
    return (
      <div className="py-20 flex flex-col items-center justify-center space-y-6 animate-in fade-in duration-500">
        ...
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-2">Scanning for best prices...</h2>
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div className="text-left">
          <h2 className="text-3xl font-black mb-1">Results for your route</h2>
      ...
      <div className="grid gap-6">
        {couriers.length > 0 ? couriers.map((courier, index) => (
          <div key={courier.courier_id || index} className="bg-bg-main border border-border-main rounded-3xl p-6 sm:p-8 shadow-sm hover:shadow-xl hover:shadow-brand-primary/5 hover:border-brand-primary/30 transition-all flex flex-col lg:flex-row lg:items-center justify-between gap-8 group relative overflow-hidden glass">
            ...
            <div className="flex items-center gap-6">
              <div className="w-16 h-16 sm:w-20 sm:h-20 bg-bg-soft rounded-2xl flex items-center justify-center font-black text-2xl text-brand-primary border border-border-main group-hover:scale-105 transition-transform shadow-inner">
                {courier.name ? courier.name[0] : '?'}
              </div>
              <div className="text-left">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <h3 className="font-black text-xl sm:text-2xl">{courier.name || 'Unknown'}</h3>
            ...
            <div className="flex gap-3">
              <button className="flex-1 lg:flex-none px-6 py-4 bg-bg-soft border border-border-main rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-bg-main hover:border-brand-primary transition-all flex items-center justify-center gap-2 active:scale-95">
                Visit Site <ExternalLink className="w-4 h-4" />
              </button>
              <button 
                onClick={() => handleBook(courier)}
                className="flex-1 lg:flex-none px-8 py-4 bg-brand-primary text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-brand-secondary transition-all shadow-lg shadow-brand-primary/20 active:scale-95"
              >
                Book with us
              </button>
            </div>
          ...
          <div className="text-center py-10">
            <p className="text-text-muted">No couriers available for this route.</p>
          </div>
        )}
      </div>

      <div className="bg-brand-primary/5 p-8 rounded-[2rem] border border-brand-primary/10 flex flex-col md:flex-row items-center gap-8 text-left animate-in slide-in-from-bottom-12 duration-700 glass">
        <div className="w-16 h-16 bg-brand-primary rounded-2xl flex items-center justify-center shrink-0 shadow-xl shadow-brand-primary/20">
          <Shield className="w-8 h-8 text-white" />
        </div>
        <div className="space-y-2">
          <h4 className="text-xl font-black">The ShipEasy Promise</h4>
          <p className="text-text-muted font-medium leading-relaxed">
            When you book through our platform, we handle the pickup scheduling, label generation, and provide direct customer support. If anything goes wrong, we're the ones who fight for you with the courier company.
          </p>
        </div>
        <button className="whitespace-nowrap px-8 py-4 bg-bg-main border-2 border-brand-primary text-brand-primary rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-brand-primary hover:text-white transition-all">
          Learn More
        </button>
      </div>
    </div>
