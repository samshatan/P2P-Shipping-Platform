      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-x-0 top-20 z-40 glass-panel border-t-0 p-6 flex flex-col gap-4 lg:hidden"
          >
            <Link to="/calculator" onClick={() => setMobileMenuOpen(false)} className="text-lg font-bold text-left">Ship Now</Link>
            <Link to="/tracking" onClick={() => setMobileMenuOpen(false)} className="text-lg font-bold text-left">Tracking</Link>
            <Link to="/bulk" onClick={() => setMobileMenuOpen(false)} className="text-lg font-bold text-left">Bulk</Link>
            <hr className="border-white/10 my-2" />
            <div className="flex flex-col gap-3">
               {!user ? (
                 <>
                   <Link to="/login" onClick={() => setMobileMenuOpen(false)} className="py-3 rounded-xl font-bold bg-white/5 border border-white/10 text-white text-center">Sign In</Link>
                   <Link to="/calculator" onClick={() => setMobileMenuOpen(false)} className="py-3 rounded-xl font-bold bg-white text-black text-center">Get Started</Link>
                 </>
               ) : (
                 <Link to="/dashboard" onClick={() => setMobileMenuOpen(false)} className="py-3 rounded-xl font-bold bg-white text-black text-center">Go to Dashboard</Link>
               )}
            </div>
          </motion.div>
        )}
