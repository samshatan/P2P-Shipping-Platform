...
/**
 * BE3 — Day 11: Updated Address Input Page
 * Updated navigation to skip Evidence Vault and go straight to Review.
 * Cleaned up technical text references.
 */
export default function AddressInput() {
  ...
  const handleContinue = async () => {
    ...
    try {
      setTimeout(() => {
        setIsLoading(false);
        navigate('/book/review'); // Skipped Evidence Vault
      }, 800);
    } catch (err) {
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <Card className="p-10 bg-card border-none rounded-[3rem] shadow-2xl ring-1 ring-border/5 relative overflow-hidden group">
              ...
              <div className="grid sm:grid-cols-2 gap-10">
                ...
                <div className="space-y-4 sm:col-span-2">
                  <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1">Full Address</Label>
                  <Input 
                ...
                <div className="space-y-4">
                  ...
                  <Input value={pickup.name} onChange={e => updatePickup({ name: e.target.value })} placeholder="John Doe" className="h-16 bg-muted/20 border-border/10 rounded-2xl font-black" />
                </div>
                <div className="space-y-4">
                  <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1">Phone</Label>
                  <Input value={pickup.phone} onChange={e => updatePickup({ phone: e.target.value.replace(/\D/g, '').slice(0, 10) })} placeholder="10-digit number" className="h-16 bg-muted/20 border-border/10 rounded-2xl font-black" />
                </div>
              </div>
            </Card>
            ...
            <div className="relative flex justify-center -my-6 z-10">
              <Button 
                ...
                variant="outline" 
                className="w-14 h-14 rounded-full bg-background shadow-xl border-border/40 hover:border-primary hover:text-primary transition-all p-0 group"
              >
              ...
              <div className="grid sm:grid-cols-2 gap-10">
                <div className="space-y-4 sm:col-span-2 relative">
                  ...
                  <div className="relative">
                    ...
                    <Input value={delivery.pincode} onChange={e => updateDelivery({ pincode: e.target.value.replace(/\D/g, '').slice(0, 6) })} placeholder="6-digit Pincode" className="pl-14 h-16 bg-muted/20 border-border/10 rounded-2xl font-black text-lg" />
                  </div>
                </div>
                <div className="space-y-4 sm:col-span-2">
                  <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1">Full Address</Label>
                  <Input value={delivery.address} onChange={e => updateDelivery({ address: e.target.value })} placeholder="Street, Building, Flat No." className="h-16 bg-muted/20 border-border/10 rounded-2xl font-black italic" />
                </div>
                <div className="space-y-4">
                  <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1">Name</Label>
                  <Input value={delivery.name} onChange={e => updateDelivery({ name: e.target.value })} placeholder="Jane Doe" className="h-16 bg-muted/20 border-border/10 rounded-2xl font-black" />
                </div>
                <div className="space-y-4">
                  <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1">Phone</Label>
                  <Input value={delivery.phone} onChange={e => updateDelivery({ phone: e.target.value.replace(/\D/g, '').slice(0, 10) })} placeholder="10-digit number" className="h-16 bg-muted/20 border-border/10 rounded-2xl font-black" />
                </div>
              </div>
            </Card>

            <div className="flex justify-between items-center pt-8 gap-8">
              <Button onClick={() => navigate('/book/courier')} variant="ghost" className="h-16 px-12 rounded-2xl font-black uppercase tracking-widest text-[10px] border border-border/20 opacity-40 hover:opacity-100">BACK</Button>
              <Button onClick={handleContinue} disabled={isLoading || !isFormValid()} className="flex-1 h-16 bg-primary text-white hover:bg-primary/90 font-black rounded-2xl text-xl shadow-xl transition-all active:scale-95 group uppercase italic tracking-tight">
            ...
            <Card className="glass-card border-none rounded-[3.5rem] p-10 shadow-2xl ring-1 ring-border/5 sticky top-28 overflow-hidden">
              ...
              <div className="space-y-10">
                <div className="flex items-start gap-4 p-5 bg-muted/20 rounded-[2rem] border border-border/10">
                    <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary shrink-0 border border-primary/20"><Zap className="w-5 h-5" /></div>
                    <div>
                ...
                <div className="flex items-start gap-4 p-5 bg-muted/20 rounded-[2rem] border border-border/10">
                    <div className="w-10 h-10 rounded-xl bg-blue-500/10 flex items-center justify-center text-blue-500 shrink-0 border border-blue-500/20"><Database className="w-5 h-5" /></div>
                    <div>
              ...
              <div className="mt-12 pt-10 border-t border-border/10">
                <p className="text-[11px] font-medium text-muted-foreground leading-relaxed italic opacity-60">All coordinates are end-to-end encrypted. Protocol integrity verified.</p>
              </div>
            </Card>
          </div>
        </div>
